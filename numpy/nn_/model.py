...
class Model:
    ...
    def fit(
        self, x, y, epochs: int = 10, batch_size: int = 100, lr: float = 0.01
    ):
        """
        ...
        self.weights.extend(
            [
                np.random.randn(X.shape[1], s)
                if i == 0
                else (np.random.randn(self.layers[i - 1], s))
                for i, s in enumerate(self.layers)
            ]
        )
        # Add biases for the weights.
        self.biases.extend(
            [np.random.randn(1, w.shape[1]) for w in self.weights]
        )
        # One-hot encoding.
        ...
        # Add output layer.
        self.weights.append(
            np.random.randn(self.weights[-1].shape[1], self.labels.shape[0])
        )
        ...
        for i in range(epochs):
            ...
            # Get batches.
            batches = [
                data[k : k + batch_size] for k in range(0, n, batch_size)
            ]
            for b in batches:
                # Unzip the batch.
                batch, labels = zip(*b)
    ...
    def eval(self, x, y):
        ...
        # Get batches.
        batches = [
            data[k : k + self.batch_size] for k in range(0, n, self.batch_size)
        ]
        acc = []
        ...
        results = sorted(
            list(zip(preds, ims)), key=lambda x: x[0], reverse=False
        )
        sample = np.array([i[1] for i in results[:16]])
        result = (
            sample.reshape(4, 4, 28, 28).swapaxes(1, 2).reshape(28 * 4, 28 * 4)
        )
        plt.imshow(result)
        plt.show()
